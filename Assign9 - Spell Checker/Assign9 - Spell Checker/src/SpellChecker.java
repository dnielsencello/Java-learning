...
//        System.out.println();
        ...
        File file = new File(System.getProperty("user.dir" ) + "\\Letter.txt");
        try (Scanner input = new Scanner(file)) {
            while (input.hasNextLine()) {
                String word = input.nextLine();
                if (!dictionary.search(word.toLowerCase(Locale.ROOT))) {
    ...
    public static BinarySearchTree<String> readDictionary() {
        ...
        try (Scanner input = new Scanner(file)) {
            while (input.hasNextLine()) {
                ...
                myArray[myArray.length - 1] = word;

            }
        } catch (java.io.IOException ex) {
            System.out.println("An error occurred trying to read the file: " + ex);
        }
        List<String> myList = Arrays.asList(myArray.clone());
        shuffle(myList);
